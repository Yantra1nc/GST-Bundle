...
define(['N/search', 'N/error', 'N/record', 'N/runtime', 'N/url', 'N/ui/serverWidget'],
	...
	function (search, error, record, runtime, url, serverWidget) {
		...
		function beforeSubmit(scriptContext) {
			...
			log.debug({title: "getIndiaSubsidiary", details: getIndiaSubsidiary});
			...
			log.debug("subsidiary: ", subsidiary);
          	var obj_IndiaSubsidiary	= getIndiaSubsidiary.toString();
			log.debug({title: "obj_IndiaSubsidiary indexOf", details: obj_IndiaSubsidiary.indexOf(subsidiary)});
			//If subsidiary matches to India(set as script parameter).
			if (obj_IndiaSubsidiary && obj_IndiaSubsidiary.indexOf(subsidiary) != -1) {
					...
					if (scriptContext.type == scriptContext.UserEventType.CREATE) {
						...
						if (lineItemCount > 0 && lineItemCount != null && lineItemCount != '') {
							...
							for (var i = 0; i < lineItemCount; i++) {
								...
								log.debug('getItem create condition:- ', getItem);
								...
								if (discountItemIds.indexOf(getItem) == -1) {
									...
									var scheduleId = billObject.getSublistValue({
										sublistId: 'item',
								...
								} //discount function getDiscount item loop end
							}
						...
						log.debug('discount Acct Ids bs :-  ', JSON.stringify(mainDiscountAcctIds));
							...
							for (var p = 0; p < lineExpenseCount; p++) {
								...
								if (discountAcctIds.indexOf(accountId) == -1) {
									...
									if (expCategory != null && expCategory != '') {
										...
										var scheduleId = billObject.getSublistValue({sublistId: 'expense',
													fieldId: 'custcol_gst_itemschedule',line: p
										});
										log.debug('exp sidd scheduleId - ', scheduleId);
						...
						if (isIGST == 'F' || isIGST == "F") {
							log.debug({title: "Inside Create Mode totalCgstAmount", details: totalCgstAmount});
							log.debug({title: "Inside Create Mode totalSgstAmount", details: totalSgstAmount});
							billObject.setValue({
							...
							log.debug({title: "Inside Create Mode totalIgstAmount", details: totalIgstAmount});
							billObject.setValue({
								fieldId: 'custbody_gst_totaligst',
		...
		function afterSubmit(scriptContext) {
			...
			try {
				...
				if (scriptContext.type == scriptContext.UserEventType.EDIT) {
					...
					if (lineItemCount > 0) {
						...
						for (var l = lineItemCount - 1; l >= 0; l--) {
							...
							if (isReversalLine) {
								log.debug({title: "Inside is reversalLine", details:"isReversalLine"});
								billObject.removeLine({sublistId: 'item',line: l});
							} 
							else {
								countlineOfItem++;
							}
					...
					for (var e = 0; e < countlineOfItem; e++) {
						...
						if (discountItemIds.indexOf(getItem) == -1) {
							...
							log.debug('getDept:- ', getDept);
							
							//My Extra Added Code
							var scheduleId = billObject.getSublistValue({
							sublistId: 'item', fieldId: 'custcol_gst_itemschedule', line: e});
							log.debug({title: "scheduleId", details: scheduleId})
							...
							if (isReversal == true && reversalProcess == true) {
								log.debug({title: "isReversal == true && reversalProcess == true", details: "First"});
							...
							//record saved normal way after edit the record then check reversal apply then it set the discount items...
								...
								log.debug({title: "isReversal == true && reversalProcess == false", details: "Second"});
							...
							//Else set the tax-code id received as is to the tax code field.
								...
								log.debug({title: "isReversal == false && isReversalLine == false", details: "Third"});
								if (isIGST == 'T' || isIGST == "T") {
					...
					for (var p = 0; p < countlineOfExp; p++) {
						...
						if (discountAcctIds.indexOf(accountId) == -1) {
							...
							if (expCategory != null && expCategory != '') {
								...
								log.debug('exp sidd scheduleId after sub- ', scheduleId);
							...
							if (isReversal == false && reversalProcess == false) {
								...
								if (isIGST == 'T' || isIGST == "T") {
								    ...
								    log.debug({title: "Entered", details:isIGST });
									billObject.selectLine({
										sublistId: 'expense',
										line: p
									});
									log.debug({title: "getTaxCode Sidd", details: getTaxCode});
									...
									log.debug({title: "cgstRate sidd", details: cgstRate});
									billObject.setCurrentSublistValue({
										sublistId: 'expense',
										fieldId: 'custcol_gst_cgstrate',
										value: cgstRate
									});
									log.debug({title: "purchaseAmountcgst sidd", details: purchaseAmountcgst});
									billObject.setCurrentSublistValue({
										sublistId: 'expense',
										fieldId: 'custcol_gst_cgstamount',
										value: purchaseAmountcgst
									});
									log.debug({title: "sgstRate sidd", details: sgstRate});
									billObject.setCurrentSublistValue({
										sublistId: 'expense',
										fieldId: 'custcol_gst_sgstrate',
										value: sgstRate
									});
									log.debug({title: "purchaseAmountsgst sidd", details: purchaseAmountsgst});
									billObject.setCurrentSublistValue({
										sublistId: 'expense',
										fieldId: 'custcol_gst_sgstamount',
										value: purchaseAmountsgst
					...
					if (isIGST == 'F' || isIGST == "F") {
						log.debug({title: "Entered Inside Settinf C & SGST", details:isIGST});
						billObject.setValue({
							fieldId: 'custbody_gst_totalcgst',
							value: totalCgstAmount
						});
						...
						log.debug({title: "Entered Inside Setting C & SGST", details:isIGST});
						billObject.setValue({
							fieldId: 'custbody_gst_totaligst',
							value: totalIgstAmount
						});
				...
				log.debug('Exception Log of record ID', exp.id);
