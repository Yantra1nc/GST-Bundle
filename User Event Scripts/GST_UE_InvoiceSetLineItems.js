...
define(['N/record', 'N/search', 'N/runtime','N/ui/serverWidget'],
    ...
    function(record, search, runtime,serverWidget) {
            ...
            if (getIndiaSubsidiary && (getIndiaSubsidiary.indexOf(subsidiary) != -1)) {
                if (runtime.executionContext === runtime.ContextType.CSV_IMPORT) {
                    ...
                    if (recordType == "salesorder") {
						var currentRecObj	= scriptContext.newRecord.id;
						log.debug({title: "currentRecObj", details:currentRecObj});
						/*var soid = scriptContext.newRecord.id;
                        log.debug('debug', 'Inside Sales Order ID:-', soid);
                        var currentRecordObj = record.load({type: scriptContext.newRecord.type,id: soid});
						*/
                    }
					...
					if(!isEnable) {
						var forGstNo	= subsidiary;
					}
					else {
						var forGstNo	= location;
					}
                    log.debug('Enitity ID' + entityId + 'forGstNo' + forGstNo + 'shippingAddress' + shippingAddress + 'currentRecordObj' + currentRecordObj);
                    setCustomerGstNumber(entityId, forGstNo, shippingAddress, currentRecordObj, isEnable);
        ...
        function afterSubmit(scriptContext) {
            ...
            if (getIndiaSubsidiary && (getIndiaSubsidiary.indexOf(subsidiary) != -1)) {
               ...
               /* if (runtime.executionContext === runtime.ContextType.CSV_IMPORT) {
                ...
                if (scriptContext.type == scriptContext.UserEventType.EDIT || scriptContext.type == scriptContext.UserEventType.CREATE || runtime.executionContext === runtime.ContextType.CSV_IMPORT) {
        ...
        function setCustomerGstNumber(customerId, forGstNo, shippingAddress, currentRecordObj, isEnable) {
            if (customerId && forGstNo) {

                log.audit('-------------Start Setting Customer GSt Number--------------------');
				if(!isEnable) {
					var subsidiaryGstNumber = searchOnSubsidiary(forGstNo);
				}
				else {
					var subsidiaryGstNumber	= searchOnLocation(forGstNo);
				}
                /*var rec = record.load({
                ...
                log.audit('Gst Number', subsidiaryGstNumber);
                ...
                if (state && state.length == 2) {
                    ...
                    stateFilter.push(search.createFilter({name: 'custrecord_gst_statesetup_stateabb',
                        operator: search.Operator.IS,
                        values: state
                    }));
					...
					var stateId			= '';
					var searchCount		= stateSearch.runPaged().count;
					log.debug({title: "searchCount", details: searchCount});
					stateSearch.run().each(function(result){
						stateId	= result.getValue({name: "custrecord_gst_statesetup_statecode"});
						return true;
					});
					log.debug({title: "stateId by SIDD", details: stateId});
					currentRecordObj.setValue({fieldId: 'custbody_gst_destinationstate', value: stateId});
					/*var rangeId = stateSearch.run().getRange({
                        start: 0,
                        end: 1
                    });
                    ...
                    if (rangeId[0]) {
                        var stateId = rangeId[0].getValue('custrecord_gst_statesetup_statecode');
						log.debug({title: "stateId by SIDD", details: stateId});
                    }*/
				...
				var destiGstNumber		= currentRecordObj.getValue({fieldId: 'custbody_gst_destinationstate'});
				
                if ((companyGstNumber && destiGstNumber) || (customerGstNumber && destiGstNumber)) {
					log.debug({title: "Entered Inside getGstType", details:"Entered"});
                    var getGstTypeId = getGstType(companyGstNumber, customerGstNumber, destiGstNumber)
                    currentRecordObj.setValue({fieldId: 'custbody_gst_gsttype', value: getGstTypeId});
                }
			}
        ...
        function getGstType(companyGstNumber, customerGstNumber, destiGstNumber) {
			log.debug({title: "Entered Inside getGstType Function", details: destiGstNumber});
            var intra = 1;
            var inter = 2;
			...
			if((Number(companyGstNumber) == destiGstNumber) && (Number(companyGstNumber) == destiGstNumber)) {
				return intra;
			}
			else {
				return inter;
